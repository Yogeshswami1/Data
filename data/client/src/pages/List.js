...
const List = () => {
  ...
  const handleStatusChange = (record) => {
    ...
    axios.put(`http://localhost:8000/api/enrollments/${record._id}`, { status: updatedStatus })
      .then(response => {
  ...
  const handleEditSubmit = () => {
    editForm.validateFields()
      .then(values => {
        ...
        axios.put(`http://localhost:8000/api/enrollments/${editRecord._id}`, values)
          .then(response => {
  ...
  const handleDeleteSelected = () => {
    const promises = selectedRowKeys.map(id => axios.delete(`http://localhost:8000/api/enrollments/${id}`));
